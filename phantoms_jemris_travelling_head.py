...
p.add_argument('-i', '--input_dir', type=str, default="./processed_data/travelling_head", help='input directory where all the data is stored in a BIDS file structure.') # required=True,
...
p.add_argument('-f', '--format', type=str, default="mat", help='Choose the files format in which the phantoms will be saved: mat, h5.') # required=True,
...
for site in sites:
    ...
    for subject in subjects:
        ...
        for session in sessions:
            ...
            target_folder = join(input_path,'phantoms')
            ...
            for file in files:
                ...
                if is_image_file(file):
                    # load PD, R1, R2* or T2 and correct for ms as required by JEMRIS
            ...
            if args.format == 'mat':
                ...
                if args.slice:
                    ...
                    save_name = '{0}_{1}_phantom_Slice{2:03d}.mat'.format(subject,session,slice_num+1)
                ...
                # if there is nothing specified
                    ...
                    save_name = '{0}_{1}_phantom_Vol.mat'.format(subject,session)
                    # if specified add R maps else use T maps
                    if args.Rmaps:
                ...
                if args.slice:
                    ...
                    save_name = '{0}_{1}_phantom_Slice{2:03d}.h5'.format(subject,session,slice_num+1)
                ...
                # if there is nothing specified
                    ...
                    save_name = '{0}_{1}_phantom_Vol.h5'.format(subject,session)
                    # format data for the phantom
